...
public class RecordSetDeserializer extends JsonDeserializer<RecordSet> {
	...
	public RecordSet deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
		...
		System.out.println("Tree in RecordSetDeserializer ================ " + tree.toString());
		...
		if (tree.has("location") && (tree.get("location").has("x") && tree.get("location").has("y"))) {
				...
				if (channelValue != null) {
					if (channelNode.isArray()) {
						for (int i = 0; i < channelNode.size(); i++) {
							String chName = channelName + "_" + i;
							Channel ch = new Channel();
							ch.name(chName);

							JsonNode s = channelNode.get(i);
							Object val = RecordSet.parseType(s, ch);
							if (val != null) {
								recordset.channel(chName, val);
							}
						}
					} else {
